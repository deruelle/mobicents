...
public class SbbVerifier {
    ...
    public SbbVerifier(SbbDescriptor sbbDescriptor) {   
        ...
        pool=((DeployableUnitIDImpl) (sbbDescriptor
    ...
    public boolean verifySbbAbstractClass(String sbbAbstractClassName, SleeContainer serviceContainer){
        ...
        try{
            ...
            try {
				ctClass = pool.get(sbbAbstractClassName).getClassPool().makeClass(sbbAbstractClassIS);
			} catch (NotFoundException e) {
            ...
            logger.error("Sbb Abstract Class validation failed for: " + sbbAbstractClassName, ioe);
            return false;
        }
    ...
    protected boolean checkSbbAbstractClassConstraints(CtClass sbbAbstractClass){
        ...
        if(methods!=null){
            for(int i=0;i<methods.length;i++){
                ...
                if(method.getName().equalsIgnoreCase("finalize")){
                    logger.error("finalize method is present in the sbb abstract class "+
                            sbbAbstractClass.getName());
                    return false;
                }
                if ( methods[i].getName().startsWith("ejb") ) {                	
                	logger.error("invalid method name detected " + methods[i].getName());                	
                	return false;
            }
                ...
                if ( methods[i].getName().startsWith("sbb") ) {
                    ...
                    for ( k = 0; k < abstractClassMethods.length; k ++ ) {
                        ...
                        if ( ctMethod.getName().equals(methods[i].getName())) break;
        }
                    // Not an abstract method so it is invalid.
                    if ( k == abstractClassMethods.length) {
                        logger.error("invalid method name determined " + methods[i].getName());
                        return false;
                    }
                }
            }
            ...
            logger.error("there is no abstract methods inthis abstract Class : "+sbbAbstractClass.getName());
            return false;
        }
        //check that the method implements the Sbb class either directly or by inheritance
        try{
            boolean implementSbbLinkFound=false;            
            implementSbbLinkFound=
                checkInterfaces(sbbAbstractClass.getInterfaces(),"javax.slee.Sbb");
            ...
            while(superClass!=null && !implementSbbLinkFound){
                if(superClass.getName().equalsIgnoreCase("javax.slee.Sbb"))
                    ...
                    implementSbbLinkFound=
                        checkInterfaces(superClass.getInterfaces(),"javax.slee.Sbb");
                    superClass=superClass.getSuperclass();
                }
            }
            if(!implementSbbLinkFound){
                logger.error("sbb abstract class "+
                        sbbAbstractClass.getName()+" doesn't implements the javax.slee.Sbb class either directly or by inheritance");
                return false;
            }
        }
        ...
        for(int i=0;i<ctFields.length;i++){
            if(  Modifier.isPublic(ctFields[i].getModifiers()) && (ctFields[i].getName().startsWith("sbb") ||
               ctFields[i].getName().startsWith("ejb"))){
                logger.error("the sbb abstract class "+
                        sbbAbstractClass.getName()+" has a field starting with sbb or ejb["+(!Modifier.isPrivate(ctFields[i].getModifiers()))+"]: "+
                        ctFields[i].getName());                
                return false;
            }
        }
    ...
    protected boolean checkSbbAgainstDeploymentDescriptor(CtClass sbbAbstractClass,SleeContainer serviceContainer){
        ...
        //Check CMP fields
        CMPField[] cmpFields=
            sbbDeploymentDescriptor.getCMPFields();
        if (logger.isDebugEnabled()) {
        ...
        GetChildRelationMethod[] getChildRelationMethods=
            sbbDeploymentDescriptor.getChildRelationMethods();
        ...
        if(getChildRelationMethods!=null){            
            for(int i=0;i<getChildRelationMethods.length;i++){
	           ...
	           if(getChildRelationMethod==null){
	                getChildRelationMethod = (CtMethod)superClassesAbstractMethods.get(methodName);
	                if(getChildRelationMethod==null){	                    
	                	logger.error("integrity compromised on sbbAbstractClass "+
	                			sbbAbstractClass.getName()+" GetChildRelationMethod "+
	                			methodName+" defined in the descriptor is missing");	                    
	                	return false;
	                }
	            }
        ...
        if (!checkProfileCMPMethods(sbbAbstractClass, abstractMethods, superClassesAbstractMethods))
            return false;
        
        //Check the presence of the activity context interface narrow method
        Class activityContextInterface=
            sbbDeploymentDescriptor.getActivityContextInterface();        
        if(activityContextInterface!=null){            
            ...
            CtMethod narrowMethod=(CtMethod)abstractMethods.get("asSbbActivityContextInterface");
            if(narrowMethod==null){
            	narrowMethod =(CtMethod)superClassesAbstractMethods.get("asSbbActivityContextInterface");
            	if(narrowMethod==null){
            		logger.error("integrity compromised on sbbAbstractClass "+
                        sbbAbstractClass.getName()+
                        " narrow method asSbbActivityContextInterface with the return type"+
                        activityContextInterfaceName+" defined in the descriptor is missing");
            		return false;
            	}
            }
            try {
                if(!narrowMethod.getReturnType().getName().equals(activityContextInterfaceName)){
                    logger.error("integrity compromised on sbbAbstractClass "+
                            sbbAbstractClass.getName()+
                            " narrow method asSbbActivityContextInterface with the return type"+
                            activityContextInterfaceName+" has in the abstract method a return type different "+
                ...
                logger.error("integrity compromised on sbbAbstractClass "+
                        sbbAbstractClass.getName()+
                        " narrow method asSbbActivityContextInterface with the return type"+
                        activityContextInterfaceName+" has a no return type");
            }
            //The method has been verified so it is removed from the abstract method map
        ...
        HashSet sbbEventEntries=
            sbbDeploymentDescriptor.getSbbEventEntries();
            ...
            while(it.hasNext()){
                ...
                if (sbbEventEntry.isFired()) {
	                ...
	                if ( fireEventMethod != null ) {
	                	...
	                	MobicentsEventTypeDescriptor eventTypeDescriptor=serviceContainer.getEventDescriptor(eventTypeID);
	                	...
	                	{
	                		logger.error("integrity compromised on sbbAbstractClass "+
	                                sbbAbstractClass.getName()+" on fire event method: "+fireEventMethod+
	                                " => no params");
	                        return false;
	                	}
	                	if(params.length!=3)
	                	{
	                	 ...
	                	 if(!Modifier.isAbstract(fireEventMethod.getModifiers()))
		                 	{
	                    ...
	                    String customACIName = sbbDeploymentDescriptor.getActivityContextInterfaceClassName();
	                    if ( customACIName != null ) { // This is a custom SBB activity context interface
	                    	if (!customACIName.equals(param.getName()) && !param.getName().equals(ActivityContextInterface.class.getName())) {
	                    		this.errorString = "bad fire method signature expected parameter[1] " + customACIName + " or " + ActivityContextInterface.class.getName() ;                		
	                    		logger.error( errorString );                	
	                    		return false;
	                    	}
	                    	
	                    } else if ( !param.getName().equals(ActivityContextInterface.class.getName())) { // Not a custom ACI
	                    ...
	                    param=params[2];
	                    if(!param.getName().equals(javax.slee.Address.class.getName()))
	                    {
	                    	this.errorString="bad fire method signature expected parameter[2] "+javax.slee.Address.class.getName();
	                    	logger.error(errorString);
	                    	return false;
	                    }
	                	//The method has been verified so it is removed from the abstract method map
	                    ...
	                    logger.error("integrity compromised on sbbAbstractClass "+
		                        sbbAbstractClass.getName()+" fireEventMethod "+
		                        methodName+" defined in the descriptor is missing");
		                return false;
		            }
        ...
        CtMethod getDefaultSbbUsageParameterSetMethod = (CtMethod)abstractMethods.get("getDefaultSbbUsageParameterSet");
        if ( getDefaultSbbUsageParameterSetMethod != null ) {
            ...
            try {
                ...
                if (logger.isDebugEnabled()) {
                    logger.debug("RETURN TYPE: " + returnType + " INTERFACE NAME: " + usageParameterInterfaceName);
                }
            ...
            if (params != null && params.length != 0  ) {
                logger.error("Unexpected parameter found" + params[0].getName());
                return false;
            }
        ...
        if(abstractMethods.size() > 0){
            logger.error("Some methods have not been verified against the sbb descriptor" +
            		...
            		" The following method(s) were not matched against the descriptor:");
                    	...
                    	//Set implemented=implementedMethods;
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("SbbAbstractClass "+sbbAbstractClass.getName()+" follows the descriptor");
        }
        return true;
    }
    ...
    //private boolean checkProfileCMPMethods(CtClass sbbAbstractClass, Map abstractMethods){
        ...
        ProfileCMPMethod[] profileCMPMethods=
            sbbDeploymentDescriptor.getProfileCMPMethods();
        if(profileCMPMethods!=null){            
	        for(int i=0;i<profileCMPMethods.length;i++){
	            ...
	            if(profileCMPMethod==null){
	            	...
	            	if(profileCMPMethod == null){
	            		logger.error("integrity compromised on sbbAbstractClass "+
	                        sbbAbstractClass.getName()+" ProfileCMPMethod "+
	                        methodName+" defined in the descriptor is missing");
	            		return false;
	            	}
	            }
	                ...
	                if(profileCMPMethod.getName().startsWith("sbb") ||
	                   profileCMPMethod.getName().startsWith("ejb")){
	                    logger.error("integrity compromised on sbbAbstractClass "+
		                        sbbAbstractClass.getName()+" ProfileCMPMethod "+
		                        methodName+" should not starts with sbb or ejb in its name");
		                return false;
	                }
	                CtClass[] parameters;
                    try {
                        ...
                        if(parameters.length!=1){
                            logger.error("integrity compromised on sbbAbstractClass "+
    		                        sbbAbstractClass.getName()+" ProfileCMPMethod "+
    		                        methodName+" should have one and only argument " +
                        ...
                        logger.error("integrity compromised on sbbAbstractClass "+
		                        sbbAbstractClass.getName()+" ProfileCMPMethod "+
		                        methodName+" should have one argument of type javax.slee.profile.ProfileID");
		                return false;
                    }
                    try {
                        ...
                        if(exceptions.length<2){                            
                            logger.error("integrity compromised on sbbAbstractClass "+
    		                        sbbAbstractClass.getName()+" ProfileCMPMethod "+
    		                        methodName+" should have both exceptions " +
                            ...
                            while(j<exceptions.length && 
                                ...
                                if(exceptions[j].getName().equals(
                            ...
                            if(!profileNameExceptionThrown){
                                logger.error("integrity compromised on sbbAbstractClass "+
        		                        sbbAbstractClass.getName()+" ProfileCMPMethod "+
        		                        methodName+" should have this exception " +
    ...
    private boolean checkCMPFieldsAgainstDeploymentDescripor(Map abstractMethods,CMPField[] cmpFields,CtClass sbbAbstractClass) {               
        for(int i=0;i<cmpFields.length;i++){
            ...
            //check that the CMP field has at least one accessor
            ...
            if(getterAccessor!=null){
                ...
                try{
                    ...
                    if(returnType.getName().equalsIgnoreCase("void")){
                        logger.error("integrity compromised on sbbAbstractClass "+
                                sbbAbstractClass.getName()+" on CMP Field "+fieldName+
                                " => getter accessor has no return type");
                        return false;
                    }
                }
                catch(NotFoundException nfe){
                ...
                try{
                    ...
                    if(parameterTypes!=null && parameterTypes.length>0){
                        logger.error("integrity compromised on sbbAbstractClass "+
                                sbbAbstractClass.getName()+" on CMP Field "+fieldName+
                                " => getter accessor has some parameters");
                        return false;
                    }
                }
                catch(NotFoundException nfe){}
            ...
            if(setterAccessor!=null){
                ...
                try{
                    ...
                    if(!returnType.getName().equalsIgnoreCase("void")){
                        logger.error("integrity compromised on sbbAbstractClass "+
                                sbbAbstractClass.getName()+" on CMP Field "+fieldName+
                                " => setter accessor has a return type "+returnType.getName());
                        return false;
                    }
                }
                catch(NotFoundException nfe){}
                ...
                try{
                    ...
                    if(parameterTypes.length!=1){
                        logger.error("integrity compromised on sbbAbstractClass "+
                                sbbAbstractClass.getName()+" on CMP Field "+fieldName+
                                " => setter accessor has the wrong number of parameters");
                        return false;
                    }
                }
                catch(NotFoundException nfe){
                    logger.error("integrity compromised on sbbAbstractClass "+
                            sbbAbstractClass.getName()+" on CMP Field "+fieldName+
                            " => setter accessor has no parameters");
                    return false;
                }
...
//          THE FIELD HAS BEEN CHECKED, REMOVING
     ...
     */
    private boolean checkEventHandlerMethods
        ...
        for ( Iterator it = sbbEventEntries.iterator(); it.hasNext();	) {
            ...
            if ( sbbEventEntry.getEventDirection() == SbbEventEntry.FIRED ) continue;
            ...
            MobicentsEventTypeDescriptor eventTypeDesc = serviceContainer.getEventDescriptor(eventTypeID);
            ...
            try {
                ...
                CtClass[] parameters =  ((CtMethod) methods[i]).getParameterTypes();
            ...
            if(sbbEventEntry.isInitial() && sbbEventEntry.getInitialEventSelectorMethod()!=null)
            {
            	...
            	String selectorNameAndSignature=selectorMethodName+"(L"+javax.slee.InitialEventSelector.class.getName().replace('.','/')+";)"+"L"+javax.slee.InitialEventSelector.class.getName().replace('.','/')+";";
            	String selectorForPoeple="public "+javax.slee.InitialEventSelector.class.getName()+" "+selectorMethodName+"("+javax.slee.InitialEventSelector.class.getName()+")";
            	...
            	if (logger.isDebugEnabled()) {
            		logger.debug(" == event selector method:"+selectorForPoeple+" has been verified.");
            	}
            	
            }
            if (logger.isDebugEnabled()) {
                logger.debug("found an event handler " + ((CtMethod)methods[i]).getName());
            }
        }
        return true;
