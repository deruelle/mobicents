...
public class SbbContextImpl implements SbbContext, Serializable {
    ...
    volatile protected SleeContainer serviceContainer;
    
    static {
        logger  = Logger.getLogger(SbbContextImpl.class );
    }
    ...
    public String[] getEventMask(ActivityContextInterface aci)
    	...
    	if ( ! sbbObject.getSbbEntity().checkAttached(acId))
    ...
    public SbbLocalObject getSbbLocalObject()
        ...
        if (this.sbbObject == null || this.sbbObject.getSbbEntity() == null ||
                ! this.sbbObject.getState().equals(SbbObjectState.READY))
    ...
    public void maskEvent(String[] eventNames, ActivityContextInterface aci)
        ...
        if ( !sbbObject.getSbbEntity().checkAttached(acId)) throw new NotAttachedException("ACI is not attached to SBB ");
