...
### $Id: run.sh,v 1.7 2007/08/20 16:34:30 eduardomartins Exp $ ###
...
# Lets look for -mc switch, which indicates that -c all -b 127.0.0.1 should be added
# if double -b are specified something wrong happens with RMIAdaptor - its not
# looking up on second -b, but rather on first one!
# '-mc' is ignored, dot know why, it works in standalone script....
SEARCH=`echo $@ | grep [-][m][c]`
echo "SEARCH $SEARCH"
ARGS=""
if [ "x$SEARCH" = "x" ]; then
	ARGS="$@"
...
echo "========================================================================="
echo "Arguments passed to jboss: $ARGS"
