...
public final class ServiceState implements Serializable {
     ...
     * @throws IllegalArgumentException if <code>state</code> is not a valid
     *        state value.
     */
    ...
    public String toString() {
        switch (state) {
            case SERVICE_INACTIVE: return "Service Inactive";
            case SERVICE_ACTIVE: return "Service Active";
            case SERVICE_STOPPING: return "Service Stopping";
            default: return "Service in Unknown and Invalid State";
        }
    }
    ...
    private Object readResolve() throws StreamCorruptedException {
        if (state == SERVICE_INACTIVE) return INACTIVE;
        if (state == SERVICE_ACTIVE) return ACTIVE;
        if (state == SERVICE_STOPPING) return STOPPING;
