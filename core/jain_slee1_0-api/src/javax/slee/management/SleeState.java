...
public final class SleeState implements Serializable {
    ...
    public String toString() {
        switch (state) {
            ...
            case SLEE_RUNNING: return "Running";
            case SLEE_STOPPING: return "Stopping";
            default: return "SLEE in Unknown and Invalid State";
        }
    }
    ...
    private Object readResolve() throws StreamCorruptedException {
        if (state == SLEE_STOPPED) return STOPPED;
        if (state == SLEE_STARTING) return STARTING;
        if (state == SLEE_RUNNING) return RUNNING;
        if (state == SLEE_STOPPING) return STOPPING;
