	...
	public void onDialogSetupEarly(javax.sip.ResponseEvent event,
		...
		if (resp.getStatusCode() == 101
				&& ((CSeqHeader) resp.getHeader(CSeqHeader.NAME)).getMethod()
						.equals(Request.INVITE) && !cancelSent)
			try {
				logger
						.info("\n----------------------------\nSending CANCEL\n-------------------------------\n");
	...
	public void onDialogTerminated(javax.sip.DialogTerminatedEvent event,
		...
		try {
			if (setupFailedReceived && setupEarlyReceived) {
				setResultPassed(" == RECEIVED setupEarly("+setupEarlyReceived+"), setupFailed("+setupFailedReceived+") and Terminated("+true+") events in order ==");
			} else {
				setResultFailed(" == DID NOT RECEIVE setupEarly("+setupEarlyReceived+"), setupFailed("+setupFailedReceived+") and Terminated("+true+") events in order ==");
			}
		} catch (Exception e) {
			TCKSbbUtils.handleException(e);
			e.printStackTrace();
