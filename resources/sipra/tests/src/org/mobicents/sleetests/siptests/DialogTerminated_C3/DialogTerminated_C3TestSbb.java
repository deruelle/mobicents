...
public abstract class DialogTerminated_C3TestSbb extends BaseTCKSbb {
	...
	protected static Logger logger = Logger
			.getLogger(DialogTerminated_C3TestSbb.class);
	...
	public void onInformation(javax.sip.ResponseEvent event,
		...
		if (resp.getStatusCode() == 101
			...
			/*try {
				logger
						.info("\n----------------------------\nSending CANCEL\n-------------------------------\n");
				Request cancel=event.getClientTransaction().createCancel();
	...
	public void onDialogSetupConfirmed(javax.sip.ResponseEvent event,
		...
		logger
				.info("=================  GOT SetupConfirmed EVENT =================== \n"
						+ event + "\n==================");
		try {
			Request bye=event.getDialog().createRequest(Request.BYE);
			ClientTransaction CT=fp.getSipProvider().getNewClientTransaction(bye);
			event.getDialog().sendRequest(CT);
		} catch (SipException e) {
			TCKSbbUtils.handleException(e);
			e.printStackTrace();
		}
	...
	public void onDialogTerminated(javax.sip.DialogTerminatedEvent event,
		...
		logger
				.info("=================  GOT DIALOG TERMINATED EVENT =================== \n"
						+ event + "\n==================");
		try {
			if (setupConfirmedReceived && setupEarlyReceived) {
				setResultPassed(" == RECEIVED setupEarly("+setupEarlyReceived+"), setupConfirmed("+setupConfirmedReceived+") and Terminated("+true+") events in order ==");
			} else {
				setResultFailed(" == DID NOT RECEIVE setupEarly("+setupEarlyReceived+"), setupConfirmed("+setupConfirmedReceived+") and Terminated("+true+") events in order ==");
			}
		} catch (Exception e) {
			TCKSbbUtils.handleException(e);
			e.printStackTrace();
	...
	public InitialEventSelector chooseThis(InitialEventSelector ies) {
		...
		if(name.equals("DialogTerminated_C3TestSbb"))
		{
	...
	public void onTimeEvent(javax.slee.facilities.TimerEvent event,
		...
		if (!(setupEarlyReceived && setupConfirmedReceived))
			try {
				setResultFailed("DIDNT RECEIVE EVENT WITHIN 20s\nsetupEarlyReceived:"+setupEarlyReceived+"\nsetupFailedReceived:"+setupConfirmedReceived);
			} catch (Exception e) {
