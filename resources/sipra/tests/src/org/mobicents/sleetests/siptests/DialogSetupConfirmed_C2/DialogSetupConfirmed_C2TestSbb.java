...
public abstract class DialogSetupConfirmed_C2TestSbb extends BaseTCKSbb {
	 ...
	 */
	protected static Logger logger = Logger.getLogger(DialogSetupConfirmed_C2TestSbb.class);
	...
	public void onStartServiceEvent(
		...
		try {
			...
			fromHeader = fp.getHeaderFactory().createFromHeader(fromAddres,
					"DialogSetupConfirmed_C2TestSbb");
	...
	public void onDialogSetupConfirmed(javax.sip.ResponseEvent event,
		...
		logger
				.info("=================  GOT DIALOG SetupConfirmed EVENT =================== \n"
						+ event + "\n==================");
		try {
			if(setupEarlyReceived)
				setResultPassed("GOT setupEarlySetup("+setupEarlyReceived+") and setupConfirmed(true)!!!");
			else
				setResultFailed("GOT setupEarlySetup("+setupEarlyReceived+") and setupConfirmed(true)!!!");
		} catch (Exception e) {
			TCKSbbUtils.handleException(e);
			e.printStackTrace();
		}
	...
	public void onSuccess(javax.sip.ResponseEvent event,ActivityContextInterface aci )
	{
		...
		if (resp.getStatusCode() == Response.OK
			...
			try {
				logger
						.info("\n----------------------------\nSending ACK\n-------------------------------\n");
				Request ack=event.getDialog().createAck(((CSeqHeader) resp.getHeader(CSeqHeader.NAME)).getSeqNumber());
				ClientTransaction ctx=fp.getSipProvider().getNewClientTransaction(ack);
				//ctx.sendRequest();
	...
	public InitialEventSelector chooseThis(InitialEventSelector ies) {
		...
		if(name.equals("DialogSetupConfirmed_C2TestSbb"))
		{
