package org.mobicents.sleetests.siptests.DialogSetupConfirmed_2;
...
public class DialogSetupConfirmed_2Test extends AbstractSleeTCKTest  implements SipListener{
	//protected static Logger logger=Logger.getLogger(SipResourceAdaptor.class);
	protected static Logger logger=Logger.getLogger(DialogSetupConfirmed_2Test.class);
	protected static SIPStackBinder binder=SIPStackBinder.getInstance();

	protected Dialog localDialog=null;
	private static boolean raSETUP=false;
	private static SipRaCreator creator=null;
	public DialogSetupConfirmed_2Test()
	{
		super();
		if(raSETUP)
			return;
	...
	public void run(FutureResult result) throws Exception {
		...
		try {
			...
			localDialog = CTInvite.getDialog();
	...
	public void processResponse(ResponseEvent respEvent) {
		...
		{
			getLog().info(" == RECEIVED OK:"+resp+" == ");
			try {
				Request ack=respEvent.getDialog().createAck( ((CSeqHeader)resp.getHeader(CSeqHeader.NAME)).getSeqNumber());
				ClientTransaction ackTX=binder.getProvider().getNewClientTransaction(ack);
				ackTX.sendRequest();
			} catch (InvalidArgumentException e) {
				getLog().info(e);
				e.printStackTrace();
		...
		else if( resp.getStatusCode()==101 &&	( ((CSeqHeader)resp.getHeader(CSeqHeader.NAME)).getMethod().equals(Request.INVITE) )    )
		{
			getLog().info(" == RECEIVED PROVISIONAL RESPONSE:"+resp);
