...
public abstract class DialogSetupConfirmed_2TestSbb extends BaseTCKSbb {
	...
	protected static Logger logger = Logger
			.getLogger(DialogSetupConfirmed_2TestSbb.class);
	...
	public void onDialogSetupConfirmed(javax.sip.ResponseEvent event,
		...
		if (getSetupEarlyReceived() && getSetupConfirmedReceived()) {
			try {
				setResultPassed("GOT SETUP Early THAN SETUP Confirmed!!!");
			} catch (Exception e) {
				TCKSbbUtils.handleException(e);
				e.printStackTrace();
			}
			...
			try {
				setResultFailed("GOT SETUP Confirmed, BUT DID NoT RECEIVE SETUP EARLY!!!");
			} catch (Exception e) {
				TCKSbbUtils.handleException(e);
				e.printStackTrace();
			}
	...
	public InitialEventSelector chooseThis(InitialEventSelector ies) {
		...
		if(name.equals("DialogSetupConfirmed_2"))
		{
