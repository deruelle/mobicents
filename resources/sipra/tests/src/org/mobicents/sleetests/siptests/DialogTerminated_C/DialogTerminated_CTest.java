package org.mobicents.sleetests.siptests.DialogTerminated_C;
...
public class DialogTerminated_CTest extends AbstractSleeTCKTest implements
		SipListener {
	// protected static Logger
	// logger=Logger.getLogger(SipResourceAdaptor.class);
	protected static Logger logger = Logger
			.getLogger(DialogTerminated_CTest.class);
	...
	protected static Dialog dial = null;
	...
	public void processRequest(RequestEvent event) {
		...
		if (req.getMethod().equals(Request.INVITE)) {
			...
			try {
				ServerTransaction STX = binder.getProvider()
						.getNewServerTransaction(req);
				dial = STX.getDialog();
				if (dial == null) {
					getLog().info(" == DIALOG NULL1 ==");
