...
public abstract class DialogTerminated_3TestSbb extends BaseTCKSbb {
	...
	protected static boolean dialogTerminationRequestReceived=false;

	protected SipFactoryProvider fp;

	protected static ServerTransaction inviteSTX = null;

	// protected boolean testSuccess = false;
	protected TimerOptions tOptions = new TimerOptions(false, 5000,
			TimerPreserveMissed.LAST);

	protected static int dtrDelivered = 0;
	protected static boolean inviteProcessed=false;
	protected static boolean setupEarlyProcessed=false;
	/*
	...
	protected static Logger logger = Logger
			.getLogger(DialogTerminated_3TestSbb.class);
	...
	public void onInvite(javax.sip.RequestEvent event,
		...
		try {
			...
			ServerTransaction stx = event.getServerTransaction();
			inviteSTX = stx;
			Response resp = fp.getMessageFactory().createResponse(101, req);

			ToHeader toHeader = (ToHeader) resp.getHeader(ToHeader.NAME);
			toHeader.setParameter("tag",
					"ALALALALALLALAALLA_TEMP_HEADER_SIP_TCK_" + Math.random()
							* 10000);
	...
	public void onDialogSetupEarly(javax.sip.ResponseEvent event,
		...
		logger
				.info("=================  GOT DIALOG SetupEarly EVENT =================== \n"
						+ event + "\n==================");
		if(setupEarlyProcessed)
			return;
		setupEarlyReceived = true;

		Request inviteRequest = inviteSTX.getRequest();
		Response okResponse = null;

		try {
			okResponse = messageFactory.createResponse(Response.OK,
					inviteRequest);
			ContactHeader contact=headerFactory.createContactHeader(((ToHeader)event.getResponse().getHeader(ToHeader.NAME)).getAddress());
			okResponse.addHeader(contact);
			inviteSTX.sendResponse(okResponse);
			setupEarlyProcessed=true;
		} catch (ParseException e) {
			TCKSbbUtils.handleException(e);
			e.printStackTrace();
		} catch (SipException e) {
	...
	public void onDialogSetupConfirmed(javax.sip.ResponseEvent event,
		...
		logger
				.info("=================  GOT DIALOG SetupConfirmed EVENT =================== \n"
						+ event + "\n==================");
		setupConfirmedReceived = true;
	...
	public void onDialogTerminationRequest(javax.sip.RequestEvent event,
			ActivityContextInterface aci) {
		logger
				.info("=================  GOT DIALOG TERMINATION REQUEST EVENT =================== \n"
						+ event + "\n==================");
		dialogTerminationRequestReceived=true;
	...
	public void onDialogTerminated(javax.sip.DialogTerminatedEvent event,
		...
		logger
				.info("=================  GOT DIALOG TERMINATED EVENT =================== \n"
						+ event + "\n==================");
		try {
			if (setupConfirmedReceived && setupEarlyReceived && dialogTerminationRequestReceived) {
				setResultPassed(" == RECEIVED SetupEarly("+setupEarlyReceived+")SetupConfirmed("+setupConfirmedReceived+"), TerminationRequest("+dialogTerminationRequestReceived+") and Dialog Terminated(true) events in order ==");
			} else {
				setResultFailed(" == RECEIVED SetupEarly("+setupEarlyReceived+")SetupConfirmed("+setupConfirmedReceived+"), TerminationRequest("+dialogTerminationRequestReceived+") and Dialog Terminated(true) events in order ==");
			}
		} catch (Exception e) {
			TCKSbbUtils.handleException(e);
			e.printStackTrace();
	...
	public InitialEventSelector chooseThis(InitialEventSelector ies) {
		...
		}else if(event instanceof DialogTerminatedEvent)
		{
			...
			name=dialog.getRemoteTag();
		}else
		{
			ies.setInitialEvent(false);
			return ies;
		}
		if(name.equals("DialogTerminated_3Test"))
		{
	...
	public void onTimeEvent(javax.slee.facilities.TimerEvent event,
			ActivityContextInterface aci) {
		if (!(setupEarlyReceived && setupConfirmedReceived))
			try {
				setResultFailed("DIDNT RECEIVE EVENT WITHIN 20s\nsetupEarlyReceived:"
						+ setupEarlyReceived
						+ "\nsetupFailedReceived:"
						+ setupConfirmedReceived);
			} catch (Exception e) {
