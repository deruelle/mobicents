package org.mobicents.sleetests.siptests.DialogTerminated_3;
...
public class DialogTerminated_3Test extends AbstractSleeTCKTest implements
		SipListener {
	// protected static Logger
	// logger=Logger.getLogger(SipResourceAdaptor.class);
	protected static Logger logger = Logger
			.getLogger(DialogTerminated_3Test.class);

	protected static boolean sentBYE=false;
	
	
	private static boolean raSETUP=false;
	private static SipRaCreator creator=null;
	public DialogTerminated_3Test()
	{
		super();
		if(raSETUP)
			return;
	...
	public void run(FutureResult result) throws Exception {
		...
		try {
			cseqHeader = binder.getHeaderFactory().createCSeqHeader(1,
					Request.INVITE);
			viaHeader = binder.getHeaderFactory().createViaHeader(
					binder.getStackAddress(), binder.getPort(),
					binder.getTransport(), null);
			Address fromAddres = binder.getAddressFactory().createAddress(
					"sip:SimpleSIPPing@" + binder.getStackAddress() + ":"
							+ binder.getPort());
			// Address
			// toAddress=addressFactory.createAddress("sip:pingReceiver@"+peerAddres+":"+peerPort);
			Address toAddress = binder.getAddressFactory().createAddress(
					"sip:" + binder.getPeerAddres() + ":"
							+ binder.getPeerPort());
			contactHeader = binder.getHeaderFactory().createContactHeader(
					fromAddres);
			toHeader = binder.getHeaderFactory()
					.createToHeader(toAddress, null);
			fromHeader = binder.getHeaderFactory().createFromHeader(
					fromAddres,
					"DialogTerminated_3Test");
			callIdHeader = binder.getProvider().getNewCallId();
			maxForwardsHeader = binder.getHeaderFactory()
					.createMaxForwardsHeader(70);
			contentTypeHeader = binder.getHeaderFactory()
					.createContentTypeHeader("text", "plain");
			Address routeAddress = binder.getAddressFactory().createAddress(
					"sip:" + binder.getPeerAddres() + ":"
							+ binder.getPeerPort());
			routeHeader = binder.getHeaderFactory().createRouteHeader(
					routeAddress);
		...
		try {
			requestURI = binder.getAddressFactory().createURI(
					"sip:" + binder.getStackAddress());
			inviteRequest = request = binder.getMessageFactory().createRequest(
	...
	public void processResponse(ResponseEvent respEvent) {
		...
		if ((resp.getStatusCode() == 101)
			...
			getLog().info(" == WAITING FOR OK(200) == ");
			//try {
			//	logger
			//			.info("\n----------------------------\nSending CANCEL\n-------------------------------\n");
				//Request cancel = inviteTX.createCancel();
				// I GET ERROR - TX ALREADY EXISTS!!!
				//ClientTransaction CTCancel = binder.getProvider()
		...
		if (resp.getStatusCode() == Response.OK && !sentBYE) {
			Dialog dial=respEvent.getDialog();
			if (cseq.getMethod().equals(Request.INVITE)) {
				getLog().info(" == GOT OK TO INVITE, SENDING BYE ==");
				Request ackRequest;
				try {
					ackRequest = dial.createRequest(Request.ACK);
					dial.sendAck(ackRequest);
				} catch (SipException e) {
					result.setError(e);
					e.printStackTrace();
				...
				Request byeRequest;
				try {
					byeRequest = dial.createRequest(Request.BYE);
					ClientTransaction CT = binder.getProvider().getNewClientTransaction(byeRequest);
					dial.sendRequest(CT);
					sentBYE=true;
				} catch (SipException e) {
					// TODO Auto-generated catch block
	...
	public void processTransactionTerminated(TransactionTerminatedEvent arg0) {
		// TODO Auto-generated method stub
		/*
		 * try { result.setFailed(0,"RECEIVED:"+arg0+"!!!"); } catch (Exception
		 * e) { result.setError(e); e.printStackTrace(); }
		 */
	}
