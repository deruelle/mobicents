package org.mobicents.sleetests.siptests.DialogSetupConfirmedTimedOut;
...
public abstract class DialogSetupConfirmedTimedOutTestSbb extends BaseTCKSbb {
	 ...
	 */
	protected static Logger logger = Logger.getLogger(DialogSetupConfirmedTimedOutTestSbb.class);
	...
	public void onInvite(javax.sip.RequestEvent event,
		...
		try {
			dial = fp.getSipProvider().getNewDialog((Transaction)aci.getActivity());
			 ...
			 Response resp=messageFactory.createResponse(Response.OK,event.getRequest());
			 ToHeader toHeader=(ToHeader) resp.getHeader(ToHeader.NAME);
			 toHeader.setTag("TCK_TMP_TAG_"+Math.random()*10000);
			 ContactHeader contact=headerFactory.createContactHeader(toHeader.getAddress());
	...
	public void onDialogSetupConfirmed(javax.sip.ResponseEvent event,
		...
		logger
				.info("=================  GOT DIALOG SETUP CONFIRMED EVENT =================== \n"
						+ event + "\n==================");
	...
	public InitialEventSelector chooseThis(InitialEventSelector ies) {
		...
		}else if(event instanceof javax.sip.DialogTerminatedEvent)
		{
			...
			logger.info("DIALOG REMOTE TAG["+name+"]");
		}else
		{
			ies.setInitialEvent(false);
			return ies;
		}
		if(name.equals("DialogSetupConfirmedTimedOut"))
		{
	...
	public abstract void setSetupConfirmedReceived(boolean flag);
