...
public class SipRaCreator implements Serializable {
	...
	public static SipRaCreator getInstance() throws Exception {
		...
		logger.info("2. raSETUP: " + creator.raSETUP);
		return creator;
	...
	public void setUpRa(FutureResult result) {
		...
		logger.info(" == STARTIGN DEPLOYMENT ==");
		if (SCI == null)
			try {
				SCI = new SleeCommandInterface(jnpHostURL);
			} catch (Exception e1) {
		...
		try {
			...
			opResult = SCI.invokeOperation("-install", dusPATH +"/ra/jars/"+ raDUName,
					null, null);
		...
		try {
			...
			if (raInstalled) {
				opResult = SCI.invokeOperation("-uninstall",
						fileURL +"/ra/jars/"+ raDUName, null, null);
