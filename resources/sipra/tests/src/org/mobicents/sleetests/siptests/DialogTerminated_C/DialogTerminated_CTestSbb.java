...
public abstract class DialogTerminated_CTestSbb extends BaseTCKSbb {
	 ...
	 */
	protected static Logger logger = Logger.getLogger(DialogTerminated_CTestSbb.class);
	...
	public void onDialogTerminated(javax.sip.DialogTerminatedEvent event,
		...
		try {
			if (setupFailed) {
				setResultPassed(" == RECEIVED setupFailed and Terminated events in order ==");
			} else {
				setResultFailed(" == DID NOT RECEIVE setupFailed and Terminated events in order ==");
			}
		} catch (Exception e) {
			TCKSbbUtils.handleException(e);
			e.printStackTrace();
	...
	public InitialEventSelector chooseThis(InitialEventSelector ies) {
		...
		if(name.equals("DialogTerminated_CTestSbb"))
		{
