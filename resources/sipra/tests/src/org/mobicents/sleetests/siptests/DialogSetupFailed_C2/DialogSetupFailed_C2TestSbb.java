package org.mobicents.sleetests.siptests.DialogSetupFailed_C2;
...
public abstract class DialogSetupFailed_C2TestSbb extends BaseTCKSbb {
	...
	protected static Logger logger = Logger
			.getLogger(DialogSetupFailed_C2TestSbb.class);
	...
	public void onStartServiceEvent(
		...
		try {
			...
			inviteRequest = request = fp.getMessageFactory().createRequest(
					...
					contentTypeHeader, "INVITE".getBytes());
	...
	public void onInformation(javax.sip.ResponseEvent event,
		...
		if(cancelSent)
		{
			logger.info("  == CANCEL SENT ONCE, ABORTING SECOND ATTEPMT ==");
			return;
		}
	...
	public void onDialogSetupFailed(javax.sip.ResponseEvent event,
		...
		if (setupEarlyReceived && setupFailedReceived) {
			try {
				setResultPassed("GOT SETUP Early THAN SETUP Failed!!!");
			} catch (Exception e) {
				TCKSbbUtils.handleException(e);
				e.printStackTrace();
			}
			...
			try {
				setResultFailed("GOT SETUP FAILED, BUT DID NoT RECEIVE SETUP EARLY!!!");
			} catch (Exception e) {
				TCKSbbUtils.handleException(e);
				e.printStackTrace();
			}
	...
	public InitialEventSelector chooseThis(InitialEventSelector ies) {
		...
		if(name.equals("DialogSetupFailed_C2Test"))
		{
