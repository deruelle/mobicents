package org.mobicents.sleetests.siptests.DialogTerminated;
...
public abstract class DialogTerminatedTestSbb extends BaseTCKSbb {
	...
	protected static Logger logger = Logger
			.getLogger(DialogTerminatedTestSbb.class);
	...
	public void onDialogTerminated(javax.sip.DialogTerminatedEvent event,
		...
		try {
			if (setupFailed) {
				setResultPassed(" == RECEIVED setupFailed("+setupFailed+") and Terminated(true) events in order ==");
			} else {
				setResultFailed(" == DID NOT RECEIVE setupFailed("+setupFailed+") and Terminated(true) events in order ==");
			}
		} catch (Exception e) {
			TCKSbbUtils.handleException(e);
			e.printStackTrace();
	...
	public InitialEventSelector chooseThis(InitialEventSelector ies) {
		...
		if(name.equals("DialogTerminatedTest"))
		{
