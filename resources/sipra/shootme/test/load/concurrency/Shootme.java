...
public class Shootme extends TestCase implements SipListener {
	...
	private class TimerTask extends java.util.TimerTask {
		...
		public TimerTask(Dialog dialog,SipProvider sipProvider) {
			if (dialog == null) {
				throw new NullPointerException("null dialog");
			}
			//System.out.println("Creating timertask for dialog "+dialog.getDialogId());
			if (sipProvider == null) {
		...
		@Override
		public void run() {
			//System.out.println("Running timertask for dialog "+dialog.getDialogId());
			this.cancel();
			try {
				//Dialog dialog = (Dialog)aci.getActivity(); 
				dialog.sendRequest(sipProvider.getNewClientTransaction(dialog.createRequest(Request.BYE)));
			} catch (Exception ex) {
				ex.printStackTrace();
			}
	...
	*/
	public void processAck(
		...
		try {
			...
			timer.schedule(new TimerTask(serverTransaction.getDialog(),sipProvider), 60000);
		} catch (Exception ex) {
			ex.printStackTrace();
