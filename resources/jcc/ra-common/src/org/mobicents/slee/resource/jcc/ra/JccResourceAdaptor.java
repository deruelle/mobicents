...
public class JccResourceAdaptor implements ResourceAdaptor, Serializable, JccConnectionListener {
    ...
    public void entityActivated() throws ResourceException {
        try {
            ...
            properties.load(getClass().getResourceAsStream("/jcc-ra.properties"));
            
            logger.info("Starting JCC Provider");
            String peerName = properties.getProperty("jcc.peer");
            System.out.println("Peername: " + peerName);
            JccPeer peer = JccPeerFactory.getJccPeer(peerName);
            
            String providerName = properties.getProperty("jcc.provider");
            System.out.println("Provider: " + provider);
            provider = new JccProviderLocal(peer.getProvider(providerName));
            
            provider.addConnectionListener(this, new BlockingFilter());
            provider.addCallListener(this);
            
            logger.info("Starting JCC Provider started succesfully");
    ...
    private void initializeNamingContext() {
        ...
        String jndiName = ((ResourceAdaptorActivityContextInterfaceFactory)
            activityContextInterfaceFactory).getJndiName();
