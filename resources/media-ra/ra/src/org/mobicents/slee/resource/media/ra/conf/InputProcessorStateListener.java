...
public class InputProcessorStateListener implements ControllerListener {
    ...
    public InputProcessorStateListener(ConfParty confParty) {
        this.confParty = confParty;
    }
    
    public void controllerUpdate(ControllerEvent evt) {
        logger.debug("(party id = " + confParty.getId() + ") evt=" + evt);
        if (evt instanceof javax.media.ConfigureCompleteEvent) {
            logger.debug("(party id = " + confParty.getId() + ") Realize receiver processor");
            confParty.realize(confParty.inputDsp, new AudioFormat(AudioFormat.ULAW_RTP, 8000, 8, 1));
        } else if (evt instanceof javax.media.RealizeCompleteEvent) {
            logger.debug("(party id = " + confParty.getId() + ") Multiplexing streams");
            confParty.mux();
