...
/**
 * This DataSourceHandler class reads from a DataSource and store
...
class DataSourceHandler implements DataSink, BufferTransferHandler {
	...
	// Contructor
	public DataSourceHandler(EventListener eventListener, MediaSession mediaSession) {
			...
			if (length == RTP_EVENT) {
    			...
    			if (octet.length() >= OCTET) {
    				...
    				if (endBit.startsWith("1")) {
    	    			...
    	    			DtmfEvent dtmfEvent = new DtmfEvent();
    	    			dtmfEvent.setDtmfDigit(dtmfDigit);
    	    			eventListener.onEvent(dtmfEvent, mediaSession);
    				}
    			}	
			}
		}	    	
