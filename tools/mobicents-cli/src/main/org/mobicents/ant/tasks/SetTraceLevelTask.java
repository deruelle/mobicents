...
public class SetTraceLevelTask implements SubTask {
    ...
    private static Logger logger = Logger.getLogger(org.mobicents.ant.tasks.SetTraceLevelTask.class.getName());
    ...
    public void run(SleeCommandInterface slee) {
		...
		try {
			...
			if (type.equalsIgnoreCase("event")) {
				...
				ComponentKey componentKey = new ComponentKey(componentID);
				component = new ResourceAdaptorTypeIDImpl(componentKey).toString();
				
			}else if(type.equalsIgnoreCase("pspec")) {
				ComponentKey componentKey = new ComponentKey(componentID);
				component = new ProfileSpecificationIDImpl(componentKey).toString();
				...
				throw new InvalidArgumentException("Unrecognized component type: " + type + "\n" + 
						" Supported component types are: " + 
						"event, ra, ratype, pspec, service and sbb");
			}
			
    		// Invoke the operation
			Object result = slee.invokeOperation("-setTraceLevel", component, level, null);
