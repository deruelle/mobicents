...
public abstract class RequestProcessorSbb implements
	...
	public WriteResult delete(ResourceSelector resourceSelector,
		...
		try {
			...
			// get document
			org.openxdm.xcap.common.datasource.Document document = dataSource
			...
			if (document == null) {
						...
						if (elementNodeList.getLength() > 1) { // MULTIPLE
							...
							if (nodeSelector.getTerminalSelector() != null) {
								...
								if (terminalSelector instanceof AttributeSelector) {
									...
									String attrName = ((AttributeSelector) terminalSelector)
											.getAttName();
				...
				if (resourceSelector.getNodeSelector() == null) {
					...
					dataSource.deleteDocument(documentSelector, document,
							nodeSelector);
				} else {
					// update document
					// create new etag
					...
					try {
						...
						dataSource.updateDocument(documentSelector, newETag,
								xml, document, nodeSelector);
						if (isLogDebugEnabled())
	...
	public WriteResult put(ResourceSelector resourceSelector, String mimetype,
		...
		try {
			...
			if (dynamicUserProvisioning) {
				...
				String[] appUsageCollections = dataSource
				...
				if (!found) {
					dataSource.addCollection(appUsage.getAUID(),
			...
			if (document == null) { // DOCUMENTS DOES NOT EXIST
				...
				// check parent exists
				String existingParent = dataSource.getExistingCollection(
								...
								if (nodeSelector.getTerminalSelector() != null) { // PUT
									...
									try {
										...
										if (terminalSelector instanceof AttributeSelector) { // PUT
											...
											// value is utf-8
											String attributeValue = XMLValidator
											...
											// get attribute name
											String attributeName = ((AttributeSelector) terminalSelector)
													.getAttName();
											// get attribute
									...
									String utf8string = XMLValidator
											.getUTF8String(contentStream);
									if (isLogDebugEnabled())
										logAsDebug("content is utf-8");
									// create XML fragment node
									Element newElement = XMLValidator
			...
			try {
				String xml = TextWriter.toString(domDocument);
				if (document == null) {
					dataSource.createDocument(documentSelector, newETag, xml);
					if (isLogDebugEnabled())
						logAsDebug("document created in data source");
				} else {
					dataSource.updateDocument(documentSelector, newETag, xml,
	...
	private Document putNewElementInDocument(InputStream contentStream,
		...
		// read and verify if content value is utf-8
		String utf8string = XMLValidator.getUTF8String(contentStream);
		if (isLogDebugEnabled())
			logAsDebug("element content is utf-8");
		// create XML fragment node
		Element newElement = XMLValidator
				.getWellFormedDocumentFragment(new StringReader(utf8string));
		if (isLogDebugEnabled())
