...
public class NoParentTest {
	...
	public void test() throws HttpException, IOException, JAXBException, InterruptedException {
		...
		Response response = client.putDocument(fakeAppUsageDocumentKey,appUsage.getMimetype(),documentContent);		
		// check response
		...
		// send put request and get response
		response = client.putElement(fakeAppUsageElementKey,elementContent);		
		// check response
		assertTrue("Response must exists",response != null);
		assertTrue("Response content must be the expected one and response code should be "+exception.getResponseStatus(),response.getCode() == exception.getResponseStatus() && response.getContent().equals(exception.getResponseContent()));
		...
		// send put request and get response
		response = client.putAttribute(fakeAttrKey,"");		
		// check response
		assertTrue("Response must exists",response != null);
		assertTrue("Response content must be the expected one and response code should be "+exception.getResponseStatus(),response.getCode() == exception.getResponseStatus() && response.getContent().equals(exception.getResponseContent()));
		...
		// send put request and get response
		response = client.putAttribute(attrKey,"");		
		// check response
		assertTrue("Response must exists",response != null);
		assertTrue("Response content must be the expected one and response code should be "+exception.getResponseStatus(),response.getCode() == exception.getResponseStatus() && response.getContent().equals(exception.getResponseContent()));
