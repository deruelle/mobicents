...
public abstract class PublicationControlSbb implements Sbb, PublicationControlSbbLocalObject {
	...
	public abstract void setParentSbbCMP(PublicationClientControlParentSbbLocalObject value);
	...
	public abstract void setImplementedChildSbbCMP(ImplementedPublicationControlSbbLocalObject value);
	private ImplementedPublicationControlSbbLocalObject getImplementedChildSbb() throws TransactionRequiredLocalException, SLEEException, CreateException {
		ImplementedPublicationControlSbbLocalObject childSbb = getImplementedChildSbbCMP();
		if (childSbb == null) {
			childSbb = (ImplementedPublicationControlSbbLocalObject) getImplementedSbbChildRelation().create();
	...
	public void newPublication(Object requestId, String entity, String eventPackage,
			String document, String contentType, String contentSubType, int expires) {
		
		if (getLogger().isDebugEnabled()) {
			getLogger().debug("new publication: entity="+entity+",eventPackage="+eventPackage);
		}
		...
		try {
			...
			try {
				unmarshalledContent = (JAXBElement) childSbb.getUnmarshaller().unmarshal(publicationStringReader);
			}
				...
				getParentSbbCMP().newPublicationError(requestId,Response.UNSUPPORTED_MEDIA_TYPE);
				entityManager.close();
				return;
			}
			...
			if (!childSbb.authorizePublication(entity,unmarshalledContent)) {
				getParentSbbCMP().newPublicationError(requestId,Response.FORBIDDEN);
				...
				Publication publication = new Publication(publicationKey,document,contentType,contentSubType);
				...
				if (expires != -1) {
					...
					TimerID timerID = timerFacility.setTimer(aci, null, System.currentTimeMillis() + ((expires+1) * 1000), 1, 1, timerOptions);
					publication.setTimerID(timerID);		
				}
				// compose document			
				ComposedPublication composedPublication = getUpdatedComposedPublication(entityManager,publication,childSbb);
				// persist data				
			...
			try {
				...
				getParentSbbCMP().newPublicationError(requestId,Response.SERVER_INTERNAL_ERROR);
	...
	private ComposedPublication getUpdatedComposedPublication(
			EntityManager entityManager, Publication publication, ImplementedPublicationControlSbbLocalObject childSbb) throws JAXBException, IOException {
		...
		if (composedPublication == null) {
			...
			composedPublication = childSbb.combinePublication(publication, composedPublication);
		}
		return composedPublication;
	}

	public void refreshPublication(Object requestId, String entity, String eventPackage, String oldETag, int expires) {

		if (getLogger().isDebugEnabled()) {
			getLogger().debug("refresh Publication: entity="+entity+",eventPackage="+eventPackage+",eTag="+oldETag+",expires="+expires);
		}
		...
		try {
			// get publication
			Publication publication = getPublication(entityManager,oldETag,entity,eventPackage);
			if (publication == null) {
				...
				getParentSbbCMP().refreshPublicationError(requestId,Response.CONDITIONAL_REQUEST_FAILED);
				entityManager.close();
				return;
			}
			...
			if (expires != -1) {
				...
				TimerID newTimerID = timerFacility.setTimer(aci, null, System.currentTimeMillis() + ((expires+1) * 1000), 1, 1, timerOptions);
				newPublication.setTimerID(newTimerID);
			}
			// persist new publication
			entityManager.persist(newPublication);	
			getLogger().info("Refreshed "+publication+ " for "+expires+" seconds");
		}
		catch (Exception e) {
			getLogger().error("failed to refresh publication",e);
			// try to send server error
			getParentSbbCMP().refreshPublicationError(requestId,Response.SERVER_INTERNAL_ERROR);
		}
	...
	public void removePublication(Object requestId, String entity, String eventPackage, String eTag) {
		
		if (getLogger().isDebugEnabled()) {
			getLogger().debug("removePublication: entity="+entity+",eventPackage="+eventPackage+",eTag="+eTag);
		}
		
		EntityManager entityManager = getEntityManager();
		try {
			// get publication
			Publication publication = getPublication(entityManager,eTag,entity,eventPackage);
			if (publication == null) {
				logger.info("can't remove publication for resource "+entity+" on event package "+eventPackage+" with eTag "+eTag+", it does not exist");
				getParentSbbCMP().removePublicationError(requestId,Response.CONDITIONAL_REQUEST_FAILED);
				entityManager.close();
				return;
			}
			...
			ComposedPublication composedPublication = removeFromComposedPublication(entityManager, publication,childSbb);			
			entityManager.flush();
			// notify subscribers
			childSbb.notifySubscribers(composedPublication);
		}
			...
			// try to send server error
			getParentSbbCMP().removePublicationError(requestId,Response.SERVER_INTERNAL_ERROR);
		}	
	...
	public void modifyPublication(Object requestId, String entity, String eventPackage, String oldETag, String document, String contentType, String contentSubType, int expires) {
		...
		try {
			...
			if (publication == null) {
				...
				getParentSbbCMP().modifyPublicationError(requestId,Response.CONDITIONAL_REQUEST_FAILED);
				entityManager.close();
				return;
			}
			...
			try {
			    ...
			    //  415 (Unsupported Media Type)
				logger.info("publication for resource "+entity+" on event package "+eventPackage+" has unsupported media type");
				getParentSbbCMP().modifyPublicationError(requestId,Response.UNSUPPORTED_MEDIA_TYPE);
				entityManager.close();
				return;
			}
			publicationStringReader.close();
			...
			// authorize publication
			if (!childSbb.authorizePublication(entity,unmarshalledContent)) {
				getParentSbbCMP().modifyPublicationError(requestId,Response.FORBIDDEN);
				logger.info("publication for resource "+entity+" on event package "+eventPackage+" not authorized");
			}
			else {
				// create new SIP-ETag
				...
				PublicationKey newPublicationKey = new PublicationKey(eTag,publication.getPublicationKey().getEntity(),publication.getPublicationKey().getEventPackage());
				Publication newPublication = new Publication(newPublicationKey,document,contentType,contentSubType);
				...
				// compose document			
				ComposedPublication composedPublication = getUpdatedComposedPublication(entityManager,newPublication,childSbb);
				// persist data			
				entityManager.persist(newPublication);	
				entityManager.persist(composedPublication);			
				// notify subscribers
			...
			getLogger().error("failed to refresh publication",e);
			// try to send server error
			getParentSbbCMP().modifyPublicationError(requestId,Response.SERVER_INTERNAL_ERROR);
		}
	...
	public boolean acceptsContentType(String eventPackage,
		...
		try {
			return getImplementedChildSbb().acceptsContentType(eventPackage, contentTypeHeader);
		} catch (Exception e) {
			logger.error(e);
			return false;
		}
	...
	public boolean authorizePublication(String entity,
		...
		try {
			return getImplementedChildSbb().authorizePublication(entity, unmarshalledContent);
		} catch (Exception e) {
			logger.error(e);
			return false;
		}
	...
	private ComposedPublication removeFromComposedPublication(
			EntityManager entityManager, Publication publication, ImplementedPublicationControlSbbLocalObject childSbb) {
		...
		.createNamedQuery("selectPublicationFromEntityAndEventPackage")
		...
		for (Iterator i = resultList.iterator();i.hasNext();) {
			...
			if (otherPublication.getPublicationKey().getETag().equals(publication.getPublicationKey().getETag())) {
				...
				if (composedPublication.getDocument() == null) {
					...
					// combine
					composedPublication = childSbb.combinePublication(otherPublication, composedPublication);
				}
			}
		}
		if (composedPublication.getDocument() == null) {
	...
	private Publication getPublication(EntityManager entityManager, String eTag, String entity,
		...
		try {
			...
			.createNamedQuery("selectPublicationFromKey")
	...
	private ComposedPublication getComposedPublication(EntityManager entityManager, String entity, String eventPackage) {
		try {
			return (ComposedPublication) entityManager.createNamedQuery("selectComposedPublicationFromEntityAndEventPackage")
			.setParameter("entity",entity)
			.setParameter("eventPackage",eventPackage)
			.getSingleResult();
		}
