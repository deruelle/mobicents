...
/**
 * JAIN SLEE Root Sbb for resource-lists Xcap application usage.  
 * @author Eduardo Martins
 *
 */
public abstract class ResourceListsAppUsageSbb extends AbstractAppUsageSbb {

	private static Logger logger = Logger
			.getLogger(ResourceListsAppUsageSbb.class);
		...
		try {
			...
			if (schemaDirURL != null) {
				...
				Schema schema = schemaContext
						.getCombinedSchema(ResourceListsAppUsage.DEFAULT_DOC_NAMESPACE);
				logger.info("Schemas loaded.");
				// create and return factory
				appUsageFactory = new ResourceListsAppUsageFactory(schema);
