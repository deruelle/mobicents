...
public class AnnouncementPkgFactory implements PkgFactory {
    ...
    public RequestedSignal createRequestedSignal(String signalName) {
        if (Announcement.PLAY.getEventName().equals(signalName)) {
    ...
    public RequestedEvent createRequestedEvent(String eventID) {
        if (Announcement.COMPLETED.getEventName().equals(eventID)) {
            return new CompletedRequestedEvent();
        }
        if (Announcement.FAILED.getEventName().equals(eventID)) {
