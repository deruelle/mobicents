...
public class CreateConnectionAction implements Callable {
     ...
     * 
     * @param crcx parameters of the connection to be created incapsulated 
    ...
    private JainMgcpResponseEvent createRtpConnection(CreateConnection crcx) {
        ...
        if (remoteSdp != null) {
            try {
                ...
                return new CreateConnectionResponse(crcx.getSource(), ReturnCode.Missing_RemoteConnectionDescriptor, null);
            }
        }
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("Created connection activity=" + connectionActivity.getID() + 
                    ", origin connection ID =" + connection.getId());
        }
        ...
        response.setSpecificEndpointIdentifier(new EndpointIdentifier(
                endpoint.getLocalName(), 
                crcx.getEndpointIdentifier().getDomainName()));
    ...
    private JainMgcpResponseEvent createLink(CreateConnection crcx) {
        ...
        //The mode of second connection is always send/recv as specified in RFC3435
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("Created connection activity=" + connectionActivity2.getID() + 
                    ", origin connection ID =" + connection2.getId());
        }
        ...
        response.setSpecificEndpointIdentifier(new EndpointIdentifier(
                endpoint.getLocalName(), 
                crcx.getEndpointIdentifier().getDomainName()));
        response.setSecondEndpointIdentifier(new EndpointIdentifier(
                endpoint2.getLocalName(), 
                crcx.getEndpointIdentifier().getDomainName()));
    ...
    public JainMgcpResponseEvent call() throws Exception {
        logger.info("Request TX= " + crcx.getTransactionHandle() + 
                ", CallID = " +  crcx.getCallIdentifier() + 
                ", Mode=" + crcx.getMode() + 
                ", Endpoint = " + crcx.getEndpointIdentifier() +
                ", Endpoint2 = " + crcx.getSecondEndpointIdentifier() + 
                ", SDP present = " + crcx.getRemoteConnectionDescriptor() != null);
        ...
        logger.info("Response TX = " + response.getTransactionHandle() + 
                ", Response: " + response.getReturnCode());
        return response;
    }
