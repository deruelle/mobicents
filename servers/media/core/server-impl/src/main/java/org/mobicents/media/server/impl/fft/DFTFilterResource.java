...
public class DFTFilterResource extends BaseResource implements MediaSink,
    ...
    public void release() {
        ...
        try {
            File dumpFile = new File(this.endpoint.getDumpDir(), this.endpoint.getTestName() + ".txt");
            if (dumpFile.exists()) {
                dumpFile.delete();
            } else {
                {
    ...
    public void transferData(PushBufferStream stream) {
        ...
        switch (this.signalBufferBitFormat) {
            ...
            case BIT_16:
                ...
                int dataPointer = 0;
                for (int i = 0; i < rawData.length; i++) {
                    rawData[i] = (data[dataPointer++] << 8) | (data[dataPointer++] & 0xff);
...
//---------- METHODS TO BE CALLED IN PREAPRE method of MediaSink
    private void determineBitSize() {
        if (this.receiveStream == null || this.getState() == MediaResourceState.NULL) {
        ...
        switch (this.bitSizeMultipllier) {
            ...
            default:
                ...
                throw new IllegalArgumentException(
                        "Source stream has more bits per sample[" + af.getSampleSizeInBits() + "] than we can HANDLE!!!!!");
