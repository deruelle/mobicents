...
public class RtpConnectionImpl extends BaseConnection {
	 ...
	 * @param connectionID
	 *            the identifier of the connection to be created
	 * @param mode
	...
	public String getLocalDescriptor() {
		...
		try {
			...
			try {
				...
				localSDP.setOrigin(sdpFactory.createOrigin(userName, sessionID, sessionVersion, networkType,
						addressType, address));
				...
				// Maximum number of Formats are 6 including the G729. The
				// default DEFAULT_INITIAL_CAPACITY of HashMap is 16.
				HashMap<Integer, Format> fmts = new HashMap<Integer, Format>(
						Format.FORMAT_HASHMAP_DEFAULT_INITIAL_CAPACITY, Format.FORMAT_HASHMAP_DEFAULT_LOAD_FACTOR);
				Set<Integer> map = rtpMap.keySet();

				for (Integer pt : map) {
					Format f = (Format) rtpMap.get(pt);
					// System.out.println("Checking format: " + f + ", class=" +
				...
				// This options forces the remote g728 side to avoid using
				// annexb, which is not supported right now
				if (g729) {
	...
	public void setRemoteDescriptor(String descriptor) throws SdpException, IOException, ResourceUnavailableException {
		...
		try {
			...
			if (logger.isDebugEnabled()) {
				logger.debug(this + " Set peer: " + peer + " Offered formats: " + offer);
			}
	...
	private HashMap<Integer, Format> subset(HashMap<Integer, Format> remote, HashMap<Integer, Format> local) {
		HashMap<Integer, Format> subset = new HashMap<Integer, Format>(Format.FORMAT_HASHMAP_DEFAULT_INITIAL_CAPACITY,
				Format.FORMAT_HASHMAP_DEFAULT_LOAD_FACTOR);
	 ...
	 */
	private Collection getPayloads(HashMap<Integer, Format> fmts) {
	...
	public void setGatherStats(boolean gatherStats) {
		...
		if (super.gatherStats) {
			// We have to put ourselves into path, in case of octets we could
			// plug into mux/demux
			// but here its more complicated, since in case of rtp we have to
