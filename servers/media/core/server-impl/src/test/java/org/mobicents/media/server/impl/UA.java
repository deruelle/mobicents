...
class UAConnectionListener implements ConnectionListener, NotificationListener {
	...
	public void onStateChange(Connection connection, ConnectionState oldState) {
		...
		if (connection.getState() == ConnectionState.OPEN) {
			...
			URL url = null;
			try {
				String fileUrl = System.getProperty("test.audio.file");
				System.out.println("File URL = " + fileUrl);
				url = new URL(fileUrl);
			} catch (MalformedURLException e1) {
				System.out
						.println("Cannot find the path to dtmf-0.wav. Make sure that you have specified correct value for property test.audio.file in your /core/server-impl/pom.xml");
				e1.printStackTrace();
			}
...
class ServerConnectionListener implements ConnectionListener {
	...
	public void onStateChange(Connection connection, ConnectionState oldState) {
		...
		if (connection.getState() == ConnectionState.OPEN) {
			...
			try {
				connection.getEndpoint().subscribe(EventID.TEST_SPECTRA, connection.getId(), null, listener);
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
