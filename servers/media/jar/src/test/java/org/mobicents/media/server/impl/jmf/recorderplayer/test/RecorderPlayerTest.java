...
public class RecorderPlayerTest extends SuperXCase implements
    ...
    protected File recorDir = new File(recordDirName + File.separator + "server" + File.separator + "default" + File.separator + "tmp");
    ...
    protected void setUp() throws Exception {
        ...
        super.setUp();
        ss = (SineStream) new SineGenerator(20, sineF, true).getStreams()[0];
    ...
    public void testADoRecord_WAV() throws Exception {
        ...
        Recorder rec = new Recorder(AudioFileFormat.Type.WAVE, TEST_DURATION);
        ...
        if (!start_r && !stop_on_reqeust) {
            doFail("Didnt receive one of events start[" + start_r + "], stop_on_req[" + stop_on_reqeust + "]");
        }

        if (!recordFile.exists() || new FileInputStream(recordFile).available() == 0) {
    ...
    public void update(RecorderEvent evt) {
        ...
        switch (evt.getEventType()) {
            ...
            case STOP_BY_REQUEST:
                if (!start_r) {
                    doFail("Rececived " + evt.getEventType() + " before start event!!");
                }
                stop_on_reqeust = true;
                break;
            case FACILITY_ERROR:
                doFail("Received " + evt.getEventType() + " with message" + evt.getMessage());
                break;
            default:
