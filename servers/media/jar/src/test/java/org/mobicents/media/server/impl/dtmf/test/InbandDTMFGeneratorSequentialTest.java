...
public class InbandDTMFGeneratorSequentialTest extends NewSuperXCase implements NotificationListener {
    ...
    protected static int TEST_DURATION = 1;
    protected SpectralAnalyser analyzer = null;
...
//    protected boolean notified = false;
    ...
    protected void setUp(int row, int column) throws Exception {
        ...
        if (toneName != null) {
            gen = new InbandGenerator(toneName, 20);
            analyzer = new SpectralAnalyser();
            analyzer.addListener(this);
            analyzer.configure(null);
        } else {
            throw new RuntimeException("Failed to obtain tone name for [" + row + "][" + column + "]");
        }
...
/*        FrequenciesHolder _row, _column;
        ...
        while (ERROR > 2) {
            ...
            if (!result && ERROR == MAX_ERROR) {
                ...
                doFail("Failed to pass test for tone[" + InbandGenerator.getToneName(currentRow, currentColumn) + "] - row[" + currentRow + "] col[" + currentColumn + "] for MAX_ERROR[" + MAX_ERROR + "] on first run[" + getReason() + "] PASSED F:" + Arrays.toString(FREQUENCIES));
            } else if (!result) {
                logger.info("This message indicates that inband generator has deviated from avg frequency, however it still works properly. This message is for info purposes only!!!\n" +
                        "tone[" + InbandGenerator.getToneName(currentRow, currentColumn) + "] - row[" + currentRow + "] col[" + currentColumn + "] for MAX_ERROR[" + MAX_ERROR + "] on first run[" + getReason() + "]");
                return;
            } else {
                ERROR -= 2;
            }
