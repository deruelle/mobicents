...
public class AudioFormat extends Format {
	...
	public static final String ALAW = "alaw";	// strange that this is lower case and ULAW is not...
	...
	// TODO: these are not used in this impl, only added for serialization compatibility.
	...
	{	
		// TODO: this calculation does not always give the exact same result as the reference implementation.
		...
		{
//			this calculation is correct for uncompressed data.  TODO: are there any formats where this is not correct?
...
//				FormatTraceUtils.traceMatches(this, format, false); // otherwise let subclass trace
//			}
			return false;
		}
...
//		if (getClass() == FormatUtils.audioFormatClass){
//			FormatTraceUtils.traceMatches(this, format, result); // otherwise let subclass trace
//		}
...
//		}
	...
	public Object clone()
	{
		return new AudioFormat(encoding, sampleRate, sampleSizeInBits,
				channels, endian, signed, frameSizeInBits, frameRate, dataType);
	...
	{	
		super.copy(f);
		final AudioFormat oCast = (AudioFormat) f;	// it has to be a AudioFormat, or ClassCastException will be thrown.
		this.sampleRate = oCast.sampleRate;
		this.sampleSizeInBits = oCast.sampleSizeInBits;
		this.channels = oCast.channels;
		this.endian = oCast.endian;
