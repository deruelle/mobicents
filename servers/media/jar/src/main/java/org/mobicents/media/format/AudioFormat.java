 ...
 * href="http://java.sun.com/products/java-media/jmf/2.1.1/apidocs/javax/media/format/AudioFormat.html"
...
public class AudioFormat extends Format {
	...
	// TODO: these are not used in this impl, only added for serialization
	// compatibility.
	...
	public long computeDuration(long length) {
		...
		if (FormatUtils.specified(this.frameRate)
				&& FormatUtils.specified(this.frameSizeInBits)) {
			frameRate = this.frameRate;
			frameSizeInBits = this.frameSizeInBits;
		} else if (FormatUtils.specified(this.sampleRate)
				&& FormatUtils.specified(this.sampleSizeInBits)
				&& FormatUtils.specified(this.channels)) {
	...
	public boolean equals(Object format) {
		...
		return this.sampleRate == oCast.sampleRate
				&& this.sampleSizeInBits == oCast.sampleSizeInBits
				&& this.channels == oCast.channels
				&& this.endian == oCast.endian && this.signed == oCast.signed
	...
	public boolean matches(Format format) {
		...
		final boolean result = FormatUtils.matches(this.sampleRate,
				oCast.sampleRate)
				&& FormatUtils.matches(this.sampleSizeInBits,
						oCast.sampleSizeInBits)
				...
				&& FormatUtils.matches(this.frameSizeInBits,
						oCast.frameSizeInBits)
