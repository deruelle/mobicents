...
public class EndpointManagementMBeanUtils implements EndpointManagementService {
	...
	public EndpointManagementMBeanUtils(MBeanServerConnection server,
			ObjectName ivrObjectName, ObjectName confObjectName,
			ObjectName annObjectName, ObjectName pktObjectName) {
		super();
		this.server = server;
	...
	public EndpointShortInfo[] getEndpointsShortInfo(EndpointType type)
		...
		String[] names;
		try {
			names = (String[]) this.server.invoke(beanName, "getEndpointNames",
					null, null);
			...
			for (int i = 0; i < names.length; i++) {
				...
				try {
					...
					inf.setName(name);
					value = this.server.invoke(beanName, "getConnectionsCount",
							paramValues, paramTypes);
					inf.setConnections(((Integer) value).intValue());
					value = this.server.invoke(beanName, "getCreationTime",
							paramValues, paramTypes);
					inf.setCreationTime(((Long) value).longValue());
					value = this.server
							.invoke(beanName, "getGatherPerformanceFlag",
									paramValues, paramTypes);
					inf.setGetherPerformance(((Boolean) value).booleanValue());
					inf.setType(type);

					value = this.server.invoke(beanName, "getNumberOfBytes",
							paramValues, paramTypes);
					inf.setNumberOfBytes(((Long) value).longValue());
					value = this.server.invoke(beanName, "getPacketsCount",
							paramValues, paramTypes);
					...
					e.printStackTrace();
					throw new ManagementConsoleException(
							"Endpoint Manager MBean [" + beanName
									+ "]  Failed due:\n"
			...
			e.printStackTrace();
			throw new ManagementConsoleException(
					"Endpoint Manager MBean ["
							+ beanName
							+ "] does not exist or failed to fetch endpoint names!!!!\n"
							+ MMSManagementMBeanUtils.doMessage(e));
		}
		return infos.toArray(new EndpointShortInfo[infos.size()]);
	}

	public ConnectionInfo getConnectionInfo(String endpointName,
			EndpointType type, String connectionId)
		...
		try {
			...
			value = this.server.invoke(beanName, "getConnectionCreationTime",
					paramValues, paramTypes);
			info.setCreationTime( ((Long) value).longValue());
			
			value = this.server.invoke(beanName, "getConnectionState",
					paramValues, paramTypes);
			info.setState((String) value);
			if(!info.getState().toLowerCase().equals("closed"))
			{
				value = this.server.invoke(beanName, "getConnectionLocalSDP",
						paramValues, paramTypes);
				info.setLocalSdp((String) value);
				value = this.server.invoke(beanName, "getConnectionRemoteSDP",
						paramValues, paramTypes);
			...
			value = this.server.invoke(beanName, "getOtherEnd",
					paramValues, paramTypes);
			info.setOtherEnd((String) value);
			value = this.server.invoke(beanName, "getConnectionMode",
					paramValues, paramTypes);
			info.setMode((String) value);
			
			value = this.server.invoke(beanName, "getNumberOfPackets",
					paramValues, paramTypes);
			info.setNumberOfPackets(((Long) value).longValue());
		} catch (NullPointerException e) {

			e.printStackTrace();
			throw new ManagementConsoleException("Endpoint Manager MBean ["
					+ beanName + "]  Failed due:\n"
					+ MMSManagementMBeanUtils.doMessage(e));
		} catch (InstanceNotFoundException e) {
	...
	public EndpointFullInfo getEndpointInfo(String endpointName,
			EndpointType type) throws ManagementConsoleException,
			...
			for (int i = 0; i < names.length; i++) {
				...
				try {
					...
					value = this.server.invoke(beanName, "getPacketsCount",
							paramValues, paramTypes);
					inf.setPackets(((Long) value).longValue());
					String[] connectionIds = (String[]) this.server.invoke(
							beanName, "getConnectionIds", paramValues,
							paramTypes);
					...
					for (String connectionId : connectionIds) {
						try {
							connectionInfos.add(this.getConnectionInfo(
									endpointName, type, connectionId));
						} catch (Exception e) {

						}
					}
					inf
							.setConnectionsInfo(connectionInfos
									.toArray(new ConnectionInfo[connectionInfos
											.size()]));
					value = this.server.invoke(beanName,
							"getRTPFacotryJNDIName", paramValues, paramTypes);
	...
	public void setGatherPerformanceData(String endpointName,EndpointType type, boolean value)
		...
		this.server.invoke(beanName, "setGatherPerformanceData",
				paramValues, paramTypes);
		} catch (NullPointerException e) {
	...
	public void setRTPFactoryJNDIName(String endpointName, EndpointType type,String jndiName)throws ManagementConsoleException, NoSuchEndpointException
	{
		...
		this.server.invoke(beanName, "setRTPFacotryJNDIName",
				paramValues, paramTypes);
		} catch (NullPointerException e) {
	...
	public void destroyConnection(String name, EndpointType type,
		...
		this.server.invoke(beanName, "destroyConnection",
				paramValues, paramTypes);
		} catch (NullPointerException e) {
	...
	public void destroyEndpoint(String name, EndpointType type)
		...
		String[] paramTypes = new String[] { "java.lang.String"};
		Object[] paramValues = new Object[] { name };
		this.server.invoke(beanName, "destroyEndpoint",
				paramValues, paramTypes);
		} catch (NullPointerException e) {
