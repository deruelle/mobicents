...
import org.mobicents.media.container.management.console.client.PropertiesInfo;
...
public class EditablePropertiesPanel extends Composite {
	...
	private void setRowStyle(int row) {
		propertiesTable.getCellFormatter().setStyleName(row, 0,
				"common-PropertiesPanel-name");
		propertiesTable.getCellFormatter().setStyleName(row, 1,
				"common-PropertiesPanel-value");
		propertiesTable.getRowFormatter().setVerticalAlign(row,
				HasVerticalAlignment.ALIGN_MIDDLE);

		if (row != 0) {
			propertiesTable.getCellFormatter().addStyleName(row, 0,
					"common-PropertiesPanel-notfirst");
			propertiesTable.getCellFormatter().addStyleName(row, 1,
					"common-PropertiesPanel-notfirst");
		}

		if (row % 2 != 0) {
			propertiesTable.getCellFormatter().addStyleName(row, 0,
					"common-PropertiesPanel-even");
			propertiesTable.getCellFormatter().addStyleName(row, 1,
					"common-PropertiesPanel-even");
		}
