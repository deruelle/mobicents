...
public class MgcpResourceAdaptor implements ResourceAdaptor, Serializable {
	 ...
	 *            contains references to the SLEE endpoint and the facilities
	 ...
	 * object is expected to close any system resources it has allocated. The
	 * SLEE may garbage collect the resource adaptor object at this time.
	 */
	...
	public void entityActivated() throws ResourceException {
		...
		mgcpActivityManager.setMgcpRaEntityName(bootstrapContext.getEntityName());
		if (logger.isInfoEnabled()) {
	 ...
	 * adaptor entity and thus is transitioning the resource adaptor entity from
	 * the Stopping state to the Inactive state.
	 */
	 ...
	 * 
	 * @param handle
	 *            the activity handle of the activity upon which the event was
	 *            fired.
	 * @param event
	 *            is the event fired on the activity. This value may be null if
	 *            no actual event was fired.
	 * @param eventID
	 *            the unique identifier of the event type fired to the SLEE. If
	 *            the event argument is null, the value of this parameter is
	 *            undefined.
	 * @param address
	 *            the default address on which the event was fired. If a null
	 *            default address was specified, this value is also null. If the
	 *            event argument is null, the value of this parameter is
	 *            undefined.
	 * @param flags
	 *            the necessary information that enables any post-event
	 ...
	 * This method need not be called by the same thread that invoked the
	 * fireEvent method for this event.
	 * 
	 * @param handle
	 *            the activity handle of the activity upon which the event was
	 *            fired.
	 ...
	 *            processing required by the resource adaptor.
	 * @param reason
	 *            informs the resource adaptor why the event processing failed.
	 */
	 ...
	 * 
	 * @param handle
	 *            the activity handle of the activity which has ended.
	 */
	 ...
	 * 
	 * @param handle
	 *            the activity handle of the activity which has been
	 *            unreferenced.
	 */
	 ...
	 * the activity and SBB attached to the activity context. If the activity is
	 * not alive the resource adaptor should call the appropriate activityEnding
	 * method on the SleeEndpoint, if the activity is still alive the resource
	 * adaptor is not expected to do anything.
	 * 
	 ...
	 * 
	 * @param handle
	 *            the activity which the SLEE is querying.
	 */
	 ...
	 * 
	 * @param the
	 *            activity handle of the requested activity.
	 ...
	 * @param activity
	 *            the activity object.
	 * @return activity handle or null if there is no handlers for specified
	 *         activity.
	 */
	 ...
	 * resource adaptor implements the <code>Marshaler</code> interface. The
	 ...
	 * 
	 * @param serviceKey
	 *            is the key of the service associated to the resource adaptor.
	 * @param eventIDs
	 *            the set of events the service identified by the service key is
	 *            interested in.
	 * @param resourceOptions
	 *            the set of resource options that map to the events that are
	 ...
	 * 
	 * @param serviceKey
	 *            the key of the service associated to the resource adaptor.
	 */
	 ...
	 * the serviceInstalled method before it can be activated. The SLEE can call
	 * this method irrespective of the state of the resource adaptor.
	 * 
	 * @param serviceKey
	 *            the key of the service associated to the resource adaptor.
	 */
	 ...
	 * 
	 * @param the
	 *            key of the service currently active in the resource adaptor.
	 */
	...
	public void processMgcpCommandEvent(JainMgcpCommandEvent event) {
		if (logger.isDebugEnabled()) {
			logger.debug(bootstrapContext.getEntityName() + " received event of type " + event.getObjectIdentifier()
					+ ". Request TX ID = " + event.getTransactionHandle());
		}
		...
		case Constants.CMD_AUDIT_CONNECTION:
			...
			processNonCreateConnectionMgcpEvent(auditConnection.getConnectionIdentifier(), event
					...
					"net.java.slee.resource.mgcp.AUDIT_CONNECTION", event);
			break;
		...
		case Constants.CMD_AUDIT_ENDPOINT:
			...
			processEndpointMgcpEvent(auditEndpoint.getEndpointIdentifier(),
					"net.java.slee.resource.mgcp.AUDIT_ENDPOINT", event);
			break;
		...
		case Constants.CMD_ENDPOINT_CONFIGURATION:
			...
			processEndpointMgcpEvent(endpointConfiguration.getEndpointIdentifier(),
					"net.java.slee.resource.mgcp.ENDPOINT_CONFIGURATION", event);
			break;
		...
		case Constants.CMD_MODIFY_CONNECTION:
			...
			processNonCreateConnectionMgcpEvent(modifyConnection.getConnectionIdentifier(), event
					.getEndpointIdentifier(), event.getTransactionHandle(),
					"net.java.slee.resource.mgcp.MODIFY_CONNECTION", event);
			break;
		...
		case Constants.CMD_RESTART_IN_PROGRESS:
			...
			processEndpointMgcpEvent(restartInProgress.getEndpointIdentifier(),
					"net.java.slee.resource.mgcp.RESTART_IN_PROGRESS", event);
			break;
	...
	public void processMgcpResponseEvent(JainMgcpResponseEvent response, JainMgcpEvent command) {
		...
		case Constants.RESP_AUDIT_CONNECTION:
			...
			processNonCreateConnectionMgcpEvent(auditConnection.getConnectionIdentifier(), auditConnection
					...
					"net.java.slee.resource.mgcp.AUDIT_CONNECTION_RESPONSE", response);
			break;
		case Constants.RESP_AUDIT_ENDPOINT:
			processEndpointMgcpEvent(((AuditEndpoint) command).getEndpointIdentifier(),
					"net.java.slee.resource.mgcp.AUDIT_ENDPOINT_RESPONSE", response);
			break;
		case Constants.RESP_CREATE_CONNECTION:
			...
			processNonCreateConnectionMgcpEvent(cId, endpointIdentifier, response.getTransactionHandle(),
					"net.java.slee.resource.mgcp.CREATE_CONNECTION_RESPONSE", response);
		...
		case Constants.RESP_ENDPOINT_CONFIGURATION:
			processEndpointMgcpEvent(((EndpointConfiguration) command).getEndpointIdentifier(),
					"net.java.slee.resource.mgcp.ENDPOINT_CONFIGURATION_RESPONSE", response);
			break;
		...
		case Constants.RESP_RESTART_IN_PROGRESS:
			processEndpointMgcpEvent(((RestartInProgress) command).getEndpointIdentifier(),
					"net.java.slee.resource.mgcp.RESTART_IN_PROGRESS_RESPONSE", response);
			break;
		default:
	 ...
	 * @param handle
	 *            the activity handle object.
	 * @param event
	 *            the Jain MGCP event object.
	 */
	...
	private void initializeNamingContext() {
		...
		ResourceAdaptorEntity resourceAdaptorEntity = (ResourceAdaptorEntity) sleeContainer
				.getResourceAdaptorEnitity(entityName);
		logger.debug("Resource Adaptor Entity: " + resourceAdaptorEntity);

		ResourceAdaptorTypeID resourceAdaptorTypeId = resourceAdaptorEntity.getInstalledResourceAdaptor().getRaType()
				.getResourceAdaptorTypeID();
		logger.debug("Resource Adaptor Type ID: " + resourceAdaptorTypeId);

		acif = new MgcpActivityContextInterfaceFactoryImpl(resourceAdaptorEntity.getServiceContainer(), this,
				entityName);
