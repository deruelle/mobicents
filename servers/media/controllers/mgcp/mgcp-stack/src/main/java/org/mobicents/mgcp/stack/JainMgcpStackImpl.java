...
public class JainMgcpStackImpl extends Thread implements JainMgcpStack , EndpointHandlerManager{
	...
	//private ConcurrentSkipListMap<String, EndpointHandler> endpointHandlers = new ConcurrentSkipListMap<String, EndpointHandler>(new StringComparator());
	private SortedMap<String, EndpointHandler> endpointHandlers = Collections.synchronizedSortedMap(new TreeMap<String, EndpointHandler>(new StringComparator()));
	...
	public JainMgcpStackImpl(InetAddress localAddress, int port) {		
		...
		//tt.scheduleAtFixedRate(new TimerTask(){
			...
			//	{
			//		logger.info("==["+(count++)+"]--"+socket.getLocalAddress()+":"+socket.getLocalPort()+"--> "+eh);
			//	}
			//}}, 10000, 10000);
	}
	...
	private void initExecutors()
	{
		...
		for(int i=0;i<this.executors.length;i++)
		{
			...
			if(executorQueueSize>0)
				this.executors[i]=new ThreadPoolQueueExecutor(1,1, new LinkedBlockingQueue<Runnable>(executorQueueSize));
			else
		...
		if(this.incomingDataBufferSize>0)
			this.eventSchedulerExecutor=new ThreadPoolQueueExecutor(1,1, new LinkedBlockingQueue<Runnable>(executorQueueSize));
		else
	...
	public synchronized EndpointHandler getEndpointHandler(String endpointId, boolean useFakeOnWildcard) {
		...
		else if(!endpointHandlers.containsKey(_endpointId ))
		{
			if(logger.isDebugEnabled())
			{
				logger.debug("Adding endpoint handler on:"+this.socket.getLocalAddress()+":"+this.socket.getLocalPort()+", using fakeId["+useFakeOnWildcard+"]  -  "+_endpointId);
			}
			eh=new EndpointHandler(this,_endpointId);
		...
		}else
		{
			if(logger.isDebugEnabled())
			{
				logger.debug("Fetching endpoint handler on: "+this.socket.getLocalAddress()+":"+this.socket.getLocalPort()+"  -  "+_endpointId);
			}
		...
		if(logger.isDebugEnabled())
			for(String key:endpointHandlers.keySet())
			{
				logger.debug("----AA--["+(count++)+"]-------"+this.socket.getLocalAddress()+":"+this.socket.getLocalPort()+"-------------- "+key+": "+endpointHandlers.get(key));
			}
	...
	public synchronized void removeEndpointHandler(String endpointId) {
		...
		if(logger.isDebugEnabled())
		{
			logger.debug("Removing EH"+this.socket.getLocalAddress()+":"+this.socket.getLocalPort()+": for:"+endpointId+" = "+eh);
		}
	...
	public synchronized EndpointHandler switchMapping(String fakeId,
		...
		if(eh==null)
		{
			//Well this means we are first, noone has return this before us so we do the switch
		...
		if(logger.isDebugEnabled())
			for(String key:endpointHandlers.keySet())
			{
				logger.debug("----AS--["+(count++)+"]-------"+this.socket.getLocalAddress()+":"+this.socket.getLocalPort()+"-------------- "+key+": "+endpointHandlers.get(key));
			}
	...
	/*
	public TransactionHandler getLocalTransaction(Integer valueOf) {
		return this.loaclTransactions.get(valueOf);
	}
	...
	public TransactionHandler getCompletedTransaction(Integer remoteTxIdIntegere) {
		return this.responseTx.get(remoteTxIdIntegere);
	}

	public TransactionHandler removeCompletedTx(Integer key) {
		return this.responseTx.remove(key);
	}
	...
	public void addCompletedTransaction(Integer valueOf,
			TransactionHandler transactionHandler) {
		this.responseTx.put(valueOf,transactionHandler);
		
	}

	public void addRemoteTxIDToLocalTxID(Integer remoteTxID, Integer localTxID) {
		this.remoteTxToLocalTxMap.put(remoteTxID, localTxID);
