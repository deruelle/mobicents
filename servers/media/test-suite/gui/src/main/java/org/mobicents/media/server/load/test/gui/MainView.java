...
public class MainView extends FrameView {
	...
	private void initComponents() {
		...
		org.jdesktop.application.ResourceMap resourceMap = org.jdesktop.application.Application.getInstance(
				org.mobicents.media.server.load.test.gui.Main.class).getContext().getResourceMap(MainView.class);
		...
		jComboBoxClientCodec.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "G711 A-law", "G711 U-law",
				"Speex nb" }));
		...
		mainPanelLayout.setHorizontalGroup(mainPanelLayout.createParallelGroup(org.jdesktop.layout.GroupLayout.LEADING)
				.add(
						mainPanelLayout.createSequentialGroup().addContainerGap().add(jLayeredPaneEchoTest,
								org.jdesktop.layout.GroupLayout.DEFAULT_SIZE, 658, Short.MAX_VALUE).addContainerGap()));
		mainPanelLayout.setVerticalGroup(mainPanelLayout.createParallelGroup(org.jdesktop.layout.GroupLayout.LEADING)
				.add(
						mainPanelLayout.createSequentialGroup().addContainerGap().add(jLayeredPaneEchoTest,
								org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, 544,
		...
		javax.swing.ActionMap actionMap = org.jdesktop.application.Application.getInstance(
				org.mobicents.media.server.load.test.gui.Main.class).getContext().getActionMap(MainView.class, this);
		...
		statusPanelLayout.setHorizontalGroup(statusPanelLayout.createParallelGroup(
				...
				org.jdesktop.layout.GroupLayout.DEFAULT_SIZE, 682, Short.MAX_VALUE).add(
				statusPanelLayout.createSequentialGroup().addContainerGap().add(statusMessageLabel).addPreferredGap(
						org.jdesktop.layout.LayoutStyle.RELATED, 498, Short.MAX_VALUE).add(progressBar,
	...
	private void jButtonStartActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_jButtonStartActionPerformed
		...
		try {
			writer = new CSVWriter(new FileWriter(CSVHeaders.CSV_FILE), CSVWriter.DEFAULT_SEPARATOR,
					CSVWriter.NO_QUOTE_CHARACTER);
			writer.writeNext(CSVHeaders.getHeaders());
		} catch (IOException e1) {
			logger.error("Failed to create the CSV File ", e1);
		...
		//Scheduled for 1 sec refresh rate
		myPeriodicTask = new MyPeriodicTask(1000L);
	...
	private void jButtonStopActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_jButtonStopActionPerformed
		...
		try {
			...
			logger.error("Closing of CSVWritter failed ", e);
		}
	...
	private class MyPeriodicTask extends Task<Void, Void> {
		...
		public void process(List<Void> ignored) {
			...
			long elpasedTimeP = testCurrentTime - previousTime;
			long elpasedTimeC = testCurrentTime - testStartTime;
			
			previousTime = testCurrentTime;
			
			int activeCount = test.getScheduler().getActiveCount();
			long completedTaskCount = test.getScheduler().getCompletedTaskCount();
			int successfullTaskCount = test.getTaskCompletedSuccessfully();
			int failedTaskCount = test.getTaskCompletedFailure();

			String[] nextRow = (new StringBuffer().append(test.getFormat()).append(CSVHeaders.CSV_SEPARATOR)
					.append(testStartTime).append(CSVHeaders.CSV_SEPARATOR)
					.append(testCurrentTime).append(CSVHeaders.CSV_SEPARATOR)
					.append(elpasedTimeP).append(CSVHeaders.CSV_SEPARATOR)
					.append(elpasedTimeC).append(CSVHeaders.CSV_SEPARATOR)					
					.append(activeCount).append(CSVHeaders.CSV_SEPARATOR)
					.append(completedTaskCount).append(CSVHeaders.CSV_SEPARATOR)
					.append(successfullTaskCount).append(CSVHeaders.CSV_SEPARATOR)
					.append(failedTaskCount).append(CSVHeaders.CSV_SEPARATOR)).toString().split(CSVHeaders.CSV_SEPARATOR);
			
			writer.writeNext(nextRow);
			try {
				writer.flush();
			} catch (IOException e) {				
				logger.error("Flushing of CVSWriter failed", e);
			}

			jTextFieldActiveCount.setText(String.valueOf(activeCount));
			jTextFieldCompletedTask.setText(String.valueOf(completedTaskCount));
			...
			jTextFieldSuccessfulTask.setText(String.valueOf(successfullTaskCount));
			jTextFieldErrorTask.setText(String.valueOf(failedTaskCount));
		}
	}
