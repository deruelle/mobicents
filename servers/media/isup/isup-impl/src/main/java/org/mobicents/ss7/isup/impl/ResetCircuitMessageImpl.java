...
public class ResetCircuitMessageImpl extends ISUPMessageImpl implements ResetCircuitMessage {
	...
	public static final MessageTypeImpl _MESSAGE_TYPE = new MessageTypeImpl(_MESSAGE_CODE_RSC);
	...
	ResetCircuitMessageImpl(Object source, Set<Integer> mandatoryCodes, Set<Integer> mandatoryVariableCodes, Set<Integer> optionalCodes, Map<Integer, Integer> mandatoryCode2Index,
	...
	protected int decodeMandatoryParameters(byte[] b, int index) throws ParameterRangeInvalidException {
		...
		if (b.length - index == 1) {
			...
			// Message Type
			if (b[index] != this._MESSAGE_CODE_RSC) {
				throw new ParameterRangeInvalidException("Message code is not: " + this._MESSAGE_CODE_RSC);
			}
			index++;
