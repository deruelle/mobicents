...
public class LoopbackAckMessageImpl extends ISUPMessageImpl implements LoopbackAckMessage {
	...
	public static final MessageTypeImpl _MESSAGE_TYPE = new MessageTypeImpl(_MESSAGE_CODE_LPA);

	private static final int _MANDATORY_VAR_COUNT = 0;

	static final int _INDEX_F_MessageType = 0;

	LoopbackAckMessageImpl(Object source, byte[] b, Set<Integer> mandatoryCodes, Set<Integer> mandatoryVariableCodes, Set<Integer> optionalCodes, Map<Integer, Integer> mandatoryCode2Index,
	...
	LoopbackAckMessageImpl(Object source, Set<Integer> mandatoryCodes, Set<Integer> mandatoryVariableCodes, Set<Integer> optionalCodes, Map<Integer, Integer> mandatoryCode2Index,
	...
	protected int decodeMandatoryParameters(byte[] b, int index) throws ParameterRangeInvalidException {
		...
		if (b.length - index == 3) {
			...
			try {
				// Message Type
				if (b[index] != this._MESSAGE_CODE_LPA) {
					throw new ParameterRangeInvalidException("Message code is not: " + this._MESSAGE_CODE_LPA);
				}
			} catch (Exception e) {
				// AIOOBE or IllegalArg
			...
			throw new IllegalArgumentException("byte[] must have three octet");
		}
	}
