...
public abstract class CallSbb implements Sbb {
    ...
    public void onCallCreated(RequestEvent evt, ActivityContextInterface aci) {
        ...
        ChildRelation rel = getDemo(to.getName());
        try {
            SbbLocalObject demo= rel.create();
            demo.setSbbPriority((byte)-10);
            msAci.attach(demo);
        } catch (Exception e) {
    ...
    public ChildRelation getDemo(String destination) {
        return null;    
    }
    
   // public abstract ChildRelation getIVRConnectionSbb();
