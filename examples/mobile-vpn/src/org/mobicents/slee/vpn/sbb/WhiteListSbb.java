 ...
 * @author Oleg Kulikov
 */
public abstract class WhiteListSbb implements Sbb {
    ...
    private Logger logger = Logger.getLogger(WhiteListSbb.class);
    
    private final static int INCOMING = 0;
    private final static int OUTGOING = 1;
    
    
    /** Creates a new instance of WhiteListSbb */
    public WhiteListSbb() {
    }
    ...
    public void onAuthorizeCallAttempt(JccConnectionEvent event, ActivityContextInterface ac) {
        ...
        ProfileID[] profiles = member.getWhiteList();
        ...
        if (profiles != null) {
            for (int i = 0; i < profiles.length; i++) {
                ...
                if (profile != null) {
                    try {
                        PatternProfileCMP pattern = getPatternProfile(profile);
                        if (matches(originating, pattern, INCOMING))  {
                            logger.info(connection.getCall() + ", Subscriber=" + member +
                                    ", Call is allowed by rule: " + pattern + ", forwarding to Screening");
        ...
        // call not authrorized, release
        logger.info(connection.getCall() + ", Subscriber=" + member +
                ", No white rules for this call. Release");
    ...
    public void onAddressAnalyze(JccConnectionEvent event, ActivityContextInterface ac) {
        ...
        if (profiles != null) {
            ...
            try {
                ...
                logger.error("System failure on getVirtualNetworkProfile(), caused by", e);
                release(connection, JccConnectionEvent.CAUSE_GENERAL_FAILURE);
                return;
            }
