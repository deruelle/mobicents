...
public abstract class BounceSbb extends CommonSbb {
	...
	public void onAnyEvent(MessageEvent event, ActivityContextInterface ac) {
		trace(Level.INFO, "BounceSbb: " + this
		...
		try {
			...
			trace(Level.INFO, "ANY Event: INIT:" + activity.getInitCounter()
	...
	public void onEndEvent(MessageEvent event, ActivityContextInterface ac) {
		...
		try {
			...
			trace(Level.INFO, "END Event: INIT:" + activity.getInitCounter()
					+ " ANY:" + activity.getAnyCounter() + " END:"
					+ activity.getEndCounter() + " Valid state: "
					+ activity.isValid(event.getMessage().getCommandId()));
		} catch (Exception e) {
	...
	public void onInitEvent(MessageEvent event, ActivityContextInterface ac) {
		...
		try {
			...
			trace(Level.INFO, "INIT Event: INIT:" + activity.getInitCounter()
					+ " ANY:" + activity.getAnyCounter() + " END:"
					+ activity.getEndCounter() + " Valid state: "
					+ activity.isValid(event.getMessage().getCommandId()));
	...
	/**
	 * Initial event selector method to check if the Event should initalize the Root SBB
	 * Uncomment the method below
	 * Also uncomment initial-event-selector-method-name element in TrainingExample1-sbb-jar.xml
	 */
